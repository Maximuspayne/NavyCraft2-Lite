...
public class CraftBuilder {
	...
	private static void detectWater(int x, int y, int z){
		...
		if((blockId == 8 || blockId == 9) && theBlock.getData() == 0){ //water
			if(y > craft.waterLevel) craft.waterLevel = 63;
			craft.waterType = 8;
			return;
		}
	...
	private static boolean createAirBubble(){
		NavyCraft.instance.DebugMessage("Adding an air bubble.", 4);
	...
	private static void createMatrix(){
		...
		if( craft.sizeX <= 1 || craft.sizeY <= 1 || craft.sizeZ <= 1 )
		{
			return;
		}
		...
		if(craft.type.forbiddenBlocks != null && craft.type.forbiddenBlocks.length > 0 && waitStopMakingThatCraft == false) {
			for(int i = 0; i < craft.type.forbiddenBlocks.length; i++) {
				if(blockType == craft.type.forbiddenBlocks[i]) {
					NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage("Forbidden block of type " + Material.getMaterial(blockType) + " found. " + 
	...
	public static boolean detect(Craft craft, int X, int Y, int Z, boolean autoShip){
		...
		if(craft.blockCount > craft.type.maxBlocks){
			NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "Unable to detect the " + craft.name + ", be sure it is not connected");
			NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + " to the ground, or maybe it is too big for this type of craft");
			NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "The maximum size is " + craft.type.maxBlocks + " blocks");
			return false;
		}
		else
			if(craft.blockCount <  craft.type.minBlocks)
			{
				if(craft.blockCount==0){
					NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "There is no " + craft.name + " here");
					NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "Be sure you are standing on a block");
				}
				else{
					NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "This " + craft.name + " is too small !");
					NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "You need to add " + (craft.type.minBlocks - craft.blockCount) + " blocks");
				}
		...
		//the recursive algorithm returned before the max block count have been reached, we have a craft !
				...
				for(Craft c: Craft.craftList){
					if(c != craft && c.captainName != null && !c.launcherOn ){
						...
						if( !((c.minX < craft.minX && c.maxX < craft.minX) || (craft.minX < c.minX && craft.maxX < c.minX )))
							if( !((c.minY < craft.minY && c.maxY < craft.minY) || (craft.minY < c.minY && craft.maxY < c.minY )))
								if( !((c.minZ < craft.minZ && c.maxZ < craft.minZ) || (craft.minZ < c.minZ && craft.maxZ < c.minZ ))){
									NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "" + c.captainName + " is already controling this " + craft.name);
									return false;
								}
					}
				}
				...
				if(craft.type.canDig && craft.type.digBlockId != 0){
					...
					if(craft.digBlockCount < digBlocksNeeded){
						NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "Not enough " + BlocksInfo.getName(craft.type.digBlockId) + " to make this " + craft.name + " move");
						NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "You need to add " + (digBlocksNeeded - craft.digBlockCount) + " more" );
						return false;
					}
				}
		...
		craft.blockCountStart = craft.blockCount;
		craft.lastBlockCount = craft.blockCount;
		craft.keelDepth = 63 - craft.minY;
		return true;
